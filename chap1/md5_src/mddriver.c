...
void MDFile PROTO_LIST((const char *, unsigned char [16]));
...
void MDString(string, digest, len)
	char *string;
	...
	unsigned char digests[16];
{
	...
	if ((file = fopen(filename, "rb")) == NULL)
		...
		MDFinal(digests, &context);

		fclose(file);
	}
}

// digest: 送信されてきたMD5ダイジェスト
// data: これからMD5hashを取る文字列データ
// data_len: data文字列の長さ
int MD5Compare(unsigned char *digest, unsigned char *data, size_t data_len) {
	MD_CTX context;
	unsigned char buf[16];
	MDInit(&context);
	MDUpdate(&context, data, data_len);
	MDFinal(buf, &context);
	for (int i = 0; i < 16; ++i) {
		printf("%02x", buf[i]);
	}
	...
	return (strcmp(digest, buf) == 0) ? 1 : 0;
}
